...
likelihood.log <- function(x, dataOut, data.name, sample_size_pop){
Like = -10^10
...
if (x[1]>=t0.low & x[1]<=t0.high & x[2]>=1 & x[7]<=0){	## new ##
	...
	result = fnRMS2011(x, epidemic.name = data.name)		## new ##
	...
	tr_diff = NULL
	for (i in 1:nr_sites)
		tr_diff = rbind(tr_diff, Wst[i,] - qnorm( (rho*sample_size_pop+0.5)/(sample_size_pop+1) ))
