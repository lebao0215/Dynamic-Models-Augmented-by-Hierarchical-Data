...
if (x[1]>=t0.low & x[1]<=t0.high & x[2]>=1 & x[7]<=0){	## new ##
	...
	# Now include TRI information
	prev.tri = result$annualFittingPrevs[2010-beginYear+1]
	inc.tri = result$annualIncid[2010-beginYear+1]
	p1 = 1-prev.tri
	p3 = (1-prev.tri)*inc.tri*(Omega.tri-beta.tri*T.tri)/365 + beta.tri*prev.tri
	p2 = prev.tri-p3

	if (p1>0 & p2>0 & p3>0)
	Like = Like + n.tri[1]*log(p1) + n.tri[2]*log(p2) + n.tri[3]*log(p3)
	if (p1<=0 | p2<=0 | p3<=0)
	Like = -10^10
}
return(Like)
