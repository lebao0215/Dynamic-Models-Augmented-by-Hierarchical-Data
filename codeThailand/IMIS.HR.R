...
IMIS.opt <- function(B, B.re, number_k){
	X_all = X_k = sample.prior(10)				# Draw initial samples from the prior distribution
	B0 = nrow(X_all)							# initial sample size
	...
	for (k in 1:number_k ){
		...
		if (k==1){
			...
			for (i in 1:dim(X_k)[1])
				loglike_all = c(loglike_all, likelihood.log(X_k[i,]))			# Calculate the likelihoods
		...
		if (k>=1){
			...
			sigma_all[[D+k-1]] = Sig2+diag(x.sd.hr^2/10^4)
	...
	prev = inc = rt = mu = NULL
	for (i in 1:B.re){
		xx = fnRMS2011(resample_X[i,], turnover)
