...
fnRMS2011 <- function(theta, epidemic.name){
...
# theta[3] is the initial log r(t) at t0; theta[4:7] are beta's
  ...
  if (epidemic.name=="ANC")   turnover=Inf
  if (epidemic.name=="FSWa")  turnover=5.392
  if (epidemic.name=="FSWb")  turnover=5.392
  if (epidemic.name=="IDU")   turnover=15
...
#Set population values before iterating
 ...
 for(t in timestepEpidemicStart:numsteps){
  ...
  # concentrated epidemic turnover
  z = z + y/turnover
  y = y - y/turnover
