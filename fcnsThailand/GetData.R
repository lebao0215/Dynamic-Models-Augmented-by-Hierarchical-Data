...
fnGetData <- function(country,site,data.id){ 

 #Reorganize data
 data <- read.csv(paste(dirData,country,data.id, ".csv" ,sep=""), header=TRUE, fill=TRUE, colClasses="character")
 index.site = which(data[,1]==site)
 index.site = sort(c(index.site, index.site+1))
 data <- data[index.site,-1]           #cut off site names
 ...
 if (is.vector(data)){
   N_table <- data[1]         #denominator
   P_table <- data[2]/100     #prevalence
 }
...
#Create data objects
dataOut <- fnGetData(country, site, data.id)
data <- dataOut$data
